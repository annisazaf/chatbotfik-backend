...
def _tanpa_spasi(nama: str) -> str:
    """Hapus semua spasi — bantu cocokkan 'techno preneurship' == 'technopreneurship'"""
    return nama.replace(" ", "")
...
def is_match(nama_khs: str, nama_kurikulum: str) -> bool:
    ...
    if len(a) > 5 and len(b) > 5:
        if a in b or b in a:
            return True
        # Cocokkan versi tanpa spasi (tangani word-break dari PDF)
    ...
    # Fuzzy matching sebagai fallback (tangani typo kecil / perbedaan ejaan)
